...
## <span class="ion ion-forward" style="transform: scale(1, -1);"></span>Services {.tabset .tabset-fade .tabset-pills .section-head}
...
## Projects<span class="ion ion-forward" style="transform: scale(-1, -1);"></span> {.tabset .tabset-fade .tabset-pills .section-head}
...
`bigrquerystorage` is an R client for the [BigQuery Storage API](https://cloud.google.com/bigquery/docs/reference/storage/). The main motivation is to replace `bigrquery::bq_table_download` in some workloads to avoid quotas and limits.
